function [ISC,ISC_persubject,ISC_persecond,W,A] = isceeg(datafile,locfile,gamma,Nsec,plotfig)
%
...
% compute cross-covariance between all subjects i and j  交叉协方差
Rij = permute(reshape(cov(X(:,:)),[D N  D N]),[1 3 2 4]); 

% compute within- and between-subject covariances  被试内部和之间的协方差
Rw =       1/N* sum(Rij(:,:,1:N+1:N*N),3);  % pooled over all subjects
Rb = 1/(N-1)/N*(sum(Rij(:,:,:),3) - N*Rw);  % pooled over all pairs of subjects 汇集所有被试

% shrinkage regularization of Rw 收缩正则化  是用于抑制过拟合的方法的统称，它通过动态调整估计参数的取值来降低模型的复杂度，以偏差的增加为代价来换取方差的下降。
Rw_reg = (1-gamma)*Rw + gamma*mean(eig(Rw))*eye(size(Rw));

% +++ If multiple stimuli are available, then Rw and Rb should be averaged over
% stimuli here prior to computing W and A +++  如果有多个刺激，那么Rw和Rb应该在两个时间段内取平均值

%在计算W和A之前的刺激

% compute correlated components W using regularized Rw, sort components by ISC 使用正则化Rw计算相关分量W，按ISC对分量排序
[W,ISC]=eig(Rb,Rw_reg); [ISC,indx]=sort(diag(ISC),'descend'); W=W(:,indx);

% compute forward model ("scalp projections") A 计算正向模型（“头皮投影”）A
A=Rw*W/(W'*Rw*W);
...
% ISC_persecond +++如果有多个刺激，则根据每个刺激计算Rij

%应在以下情况下使用，以计算ISC_persubject，以及ISC_persecond 

% Compute ISC resolved by subject, see Cohen et al.计算受试者解析的ISC
for i=1:N
...
if plotfig
    if ~exist('topoplot') | ~exist('notBoxPlot')
        ...
        for i=1:Ncomp
            subplot(2,Ncomp,i);
            topoplot(double(A(:,i)),locfile,'electrodes','off'); title(['a_' num2str(i)])
        end
